...
class Transaksi extends Connection
{
	...
	public function LihatGrafikPemasukanPertahun()
	{
		...
		while ($t = mysqli_fetch_array($tahun)) {
			$thn = $t['tahun'];
			$pemasukan = mysqli_query($this->connection, "select sum(nominal_transaksi) as total_pemasukan from transaksi where jenis_transaksi='Pemasukan' and year(tanggal_transaksi)='$thn'");
			$pem = mysqli_fetch_assoc($pemasukan);
			$total = $pem['total_pemasukan'];
			if ($pem['total_pemasukan'] == "") {
				$data .= "0,";
